...
def compute_similarity(row, sent_vec, i=0):
	title_sim = 0
...
def get_most_similar_answer(sentences, sent_vecs, df):
	if sentences == 0:
		...
		title_fp_sim = df.apply(lambda row: compute_similarity(row, sent_vecs[0], 1), axis=1)
	return df.loc[title_fp_sim.idxmax()].Answer


def find_answer(question, forum_response, tuto_response):
	print("FORUM RESPONSE: ", forum_response)
	try:
		answers_df = pd.DataFrame([[tokenizer.tokenize(forum_response[0])], [tokenizer.tokenize(tuto_response)]], columns=['Answer'])
		...
		if answer == forum_response[0]:         #best sentence
			return forum_response[1]            #sentences to give back (according to max_sentences)
		else:
			return tuto_response                #response from tuto
	except:
		return "I'm sorry I didn't find anything, would you like to ask another question?"

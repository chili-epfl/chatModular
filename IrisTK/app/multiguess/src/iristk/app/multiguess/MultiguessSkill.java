...
public class MultiguessSkill extends Skill {
	...
	private static Logger logger = IrisUtils.getLogger(MultiguessSkill.class); 
	
	private MultiguessFlow flow;
	private File propertiesFile;
	private String name = "MultiguessSkill";
	...
	public void init() throws Exception {
		if (recognizer.equals(RECOGNIZER_GRAMMAR))  {
			getSkillHandler().loadContext("default", new SpeechGrammarContext(new SRGSGrammar(getPackageFile("MultiguessGrammar.xml"))));
			...
			getSkillHandler().loadContext("default", new SemanticGrammarContext(new SRGSGrammar(getPackageFile("MultiguessGrammar.xml"))));
			getSkillHandler().setDefaultContext("default");
		}
		flow = new MultiguessFlow(getSkillHandler().getSystemAgentFlow());
	}
